...
async function checkProducts() {
  try {
    ...
    console.log(`✅ Produits actifs (deletedAt = null): ${activeProducts.length}`);
    console.log(`🗑️  Produits supprimés (deletedAt ≠ null): ${deletedProducts.length}\n`);
    ...
    if (deletedProducts.length > 0) {
      ...
      deletedProducts.forEach(p => {
        console.log(`  - ID: ${p.id} | Nom: ${p.name} | Supprimé le: ${p.deletedAt}`);
      });
    }
