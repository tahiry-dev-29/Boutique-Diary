...
export default function ReviewItem({ review, onReaction }: ReviewItemProps) {
  return (
    <div className="py-8 border-b border-border last:border-0 animate-in fade-in slide-in-from-bottom-2 duration-500">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-2">
              ...
              {review.isVerified && (
                <div className="bg-blue-50 text-blue-600 rounded-full p-0.5 dark:bg-blue-900/30 dark:text-blue-400">
          ...
          {["LIKE", "LOVE"].map(type => {
            ...
            return (
              <button
                ...
                className="text-xs px-2.5 py-1 rounded-full bg-muted text-muted-foreground hover:bg-muted/80 hover:text-foreground transition-all flex items-center gap-1.5 border border-transparent hover:border-border"
              >
      ...
      {review.replies && review.replies.length > 0 && (
        <div className="mt-4 space-y-4 ml-8 pl-6 border-l-2 border-border">
          {review.replies.map((reply: any) => (
            <div key={reply.id} className="space-y-2">
              <div className="flex items-center gap-2">
                <Avatar className="w-6 h-6 ring-1 ring-border shadow-sm transition-transform hover:scale-110">
