...
import { DollarSign, ShoppingBag, Users, Percent, TrendingUp, TrendingDown } from "lucide-react";
...
export function DashboardStats({ products }: DashboardStatsProps) {
  ...
  const inStockCount = products.filter(p => {
      const stock = p.stock || p.images?.reduce((acc, img) => acc + (typeof img === 'string' ? 0 : (img.stock || 0)), 0) || 0;
      return stock > 0;
  }).length;
  ...
  const stats = [
    {
      ...
      value: new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'MGA' }).format(totalValue),
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
      {stats.map((stat, index) => (
        <Card key={index} className="border-none shadow-[0_2px_10px_-2px_rgba(0,0,0,0.05)] bg-white rounded-2xl hover:shadow-[0_8px_30px_-4px_rgba(0,0,0,0.1)] transition-all duration-300">
