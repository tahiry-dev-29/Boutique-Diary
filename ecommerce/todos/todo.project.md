C'est parti Tahiry ! Voici la **Feuille de route technique complète**, adaptée pour la stack **Net.js**, formatée eactement selon ton style.

C'est une "Checklist de Combat" pour un clone PrestaShop sérieu (Headless Commerce). Copie-colle ça dans ton Obsidian ou ton gestionnaire de tâches. 🚀

---

# #Backend (NestJS + Prisma)

_(Cette partie reste inchangée car ton API NestJS sert de source de vérité pour le Net.js)_
...
- [] Modèle Prisma `AttributeGroup` et `AttributeValue` (e: Taille -> L, L).
...
- [] DTO `CreateProductInput` (Complee: incluant features et infos de base).
...
### 💰 Pricing Engine (Moteur de Pri)

- [] Modèle Prisma `SpecificPrice` (Règles de réduction).
- [] Modèle Prisma `TaRule` (TVA par pays).
- [] Service `PriceCalculationService` (Le cerveau du pri).
  - [] Logique: Pri Base + Impact Combinaison.
  - [] Logique: Application de la réduction `CustomerGroup`.
  - [] Logique: Recherche de `SpecificPrice` (Date, Quantité, Pays).
  - [] Logique: Calcul TTC (Tae).
...
# #Frontend (Net.js 15+ - App Router)

### 🏗️ Core & Architecture

- [ ] Configurer l'application `storefront` (Net.js App Router).
- [ ] Configurer Tailwind CSS & `cn` utility (cls + tailwind-merge).
...
- [ ] Configurer `NetAuth.js` (v5) ou gestion manuelle des Cookies JWT (Middleware).
...
- [ ] Page `products/[slug]/page.ts`.
  ...
  - [ ] Composant `ProductGallery` (Optimisation `net/image`).
...
### 🎨 UI Kit (Design System - React/Tailwind)
